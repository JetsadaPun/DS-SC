...
public class Trie {
    ...
    public void PrintWord() {
        for (Character key : root.children.keySet()) {
            PrintWord(root.children.get(key), key.toString());
        }
    }

    public void PrintWord(Node A, String word) {
        if (A.flag) {
            System.out.print(word + "  ");
        }
        for (Character key : A.children.keySet()) {
            PrintWord(A.children.get(key), word + key);
        }
    }
